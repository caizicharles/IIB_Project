...
class BaseModel(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.cnn = CNNLayer(input_size=2,#self.features_num + 1,
                            hidden_size=self.hidden_dim,
                            num_layers=self.layer_num)

        # self.head = Encoder(input_dim=self.hidden_dim,
    ...
    #     x = torch.zeros(1, self.features_num + 1, input_length)
    ...
    def forward(self, ecg, features, spectrogram):
        ...
        ecg = ecg.unsqueeze(dim=-1)#.to(torch.float32)
        x = torch.cat((ecg, rate), dim=-1).to(torch.float32)
        # x = ecg
